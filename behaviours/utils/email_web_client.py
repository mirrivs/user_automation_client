...
from behaviours.utils.behaviour import BehaviourException
from behaviours.utils.email_manager import EmailManager
from behaviours.utils.selenium_driver import SeleniumDriver
    ...
    def _open_email_by_subject(self, subject_text: str):
        ...
        if self.type == EmailClient.OWA:
            element = self.wait().until(EC.element_to_be_clickable(
                (By.XPATH, f"//span[contains(@class, 'lvHighlightAllClass') and contains(text(), '{safe_subject}')]")))
        elif self.type == EmailClient.O365:
            element = self.wait().until(EC.element_to_be_clickable(
                (By.XPATH, f"//span[contains(@title, '{safe_subject}')]")))
        elif self.type == EmailClient.ROUNDCUBE:
            element = self.wait().until(EC.element_to_be_clickable(
                (By.XPATH, f"//td[contains(@class, 'subject')]//a[contains(text(), '{safe_subject}')]")))
        else:
...
class O365Client(BaseEmailWebClient):
    ...
    def get_unread_emails(self):
        ...
        try:
            ...
            if not is_filtered_unread:
                ...
                self.wait(10).until(EC.element_to_be_clickable((By.XPATH, "//div[@role='menuitemradio' and @title='Unread']"))).click()
    ...
    def reply_to_email(self, subject, email_body):
        ...
        try:
            self.wait().until(EC.element_to_be_clickable(
                (By.XPATH, "//div[contains(@aria-label, 'Reply all')]"))).click()
            self.wait().until(EC.element_to_be_clickable((By.XPATH, "//button[contains(@aria-label, 'Expand header')]"))).click()
...
def getEmailClient(email_client: str | EmailClient) -> (OutlookWebAccessClient | RoundcubeClient | O365Client):
